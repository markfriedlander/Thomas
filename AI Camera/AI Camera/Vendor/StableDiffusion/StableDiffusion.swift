...
nonisolated public struct DenoiseIterator: Sequence, IteratorProtocol {
...
nonisolated public protocol ImageGenerator {
    func ensureLoaded()
...
/// - use ``SDImage`` to save the image
nonisolated public protocol TextToImageGenerator: ImageGenerator {
...
/// - ``ImageGenerator/decode(xt:)`` or ``ImageGenerator/detachedDecoder()`` to convert the final latent into an image
/// - use ``SDImage`` to save the image
nonisolated public protocol ImageToImageGenerator: ImageGenerator {
    func generateLatents(image: MLXArray, parameters: EvaluateParameters, strength: Float)
        -> DenoiseIterator
}

nonisolated enum ModelContainerError: LocalizedError {
...
public actor SDModelContainer<M> {
    ...
    /// create a ``SDModelContainer`` that supports ``TextToImageGenerator``
    ...
    ) throws -> SDModelContainer<TextToImageGenerator> {
    ...
    /// create a ``SDModelContainer`` that supports ``ImageToImageGenerator``
    static public func createImageToImageGenerator(
        configuration: StableDiffusionConfiguration, loadConfiguration: LoadConfiguration = .init()
    ) throws -> SDModelContainer<ImageToImageGenerator> {
...
nonisolated open class StableDiffusion {
...
nonisolated open class StableDiffusionBase: StableDiffusion, TextToImageGenerator {
...
nonisolated open class StableDiffusionXL: StableDiffusion, TextToImageGenerator, ImageToImageGenerator {
