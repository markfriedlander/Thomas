...
nonisolated enum SDImageError: LocalizedError {
    case failedToSave
    case unableToOpen
...
nonisolated public struct SDImage {

    public let data: MLXArray

    /// Create an SDImage from a MLXArray with ndim == 3
    public init(_ data: MLXArray) {
        precondition(data.ndim == 3)
        self.data = data
    }

    /// Create an SDImage by loading from a file
    public init(url: URL, maximumEdge: Int? = nil) throws {
        guard let source = CGImageSourceCreateWithURL(url as CFURL, nil),
            let image = CGImageSourceCreateImageAtIndex(source, 0, nil)
        else {
            throw SDImageError.unableToOpen
        }
    ...
    public func save(url: URL) throws {
        ...
        if !CGImageDestinationFinalize(destination) {
            throw SDImageError.failedToSave
        }
    }
}
