...
final class DarkRoomWorker {
    ...
    private func loop() async {
        // We do NOT keep the screen awake during a develop (Mark, 2026-08-01): the app must not hijack
        // the user's control over sleeping their phone. If the screen sleeps mid-develop, the app
        // backgrounds and the lifecycle handler stops the queue at a safe atomic boundary WITHOUT a
        // partial save; the durable record is untouched, so foregrounding re-develops the shot whole
        // (steps are atomic). That is the whole recovery story - respect the user's sleep, resume on wake.
